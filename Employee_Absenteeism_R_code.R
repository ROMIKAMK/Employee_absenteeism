...
setwd("F:/Edwisor/Employee_Absenteeism")
...
cont_var = c('Distance.from.Residence.to.Work', 'Service.time', 'Age',
...
for (i in 1:length(cont_var))
{
  assign(paste0("gn",i), ggplot(aes_string(y = (cont_var[i]), x = "Absenteeism.time.in.hours"), data = subset(df))+ 
           ...
           geom_boxplot(outlier.colour="yellow", fill = "blue" ,outlier.shape=18,
                        outlier.size=1, notch=FALSE) +
           theme(legend.position="bottom")+
           labs(y=cont_var[i],x="Absenteeism.time.in.hours")+
           ggtitle(paste("Box plot of absenteeism for",cont_var[i])))
}
...
cat_var = c('ID','Reason.for.absence','Disciplinary.failure', 
